...
public class Control 
{
	...
	public ArrayList<List> summarizeCourseData(int selectedCourse) 
	{
		...
		//temp
		ArrayList<Courses> list_Courses = getCourses();
		...
		for (int i = 0; i < list_Folder.size(); i++) {
			...
			//somehow there is the fisr elemnt false so we remove it
	...
	public void addToDownload(int selected) 
	{
		downloadList.add(summerizedList.get(selected));
	}
	
	public void removeFromDownload(int selected) 
	{
		
		if (downloadList.get(selected).isFolder() == true) {
			 //count Files in Folder and remove them until list is finished or next Folder
   		   
			int x=0;
			int downloadAnzahl = downloadList.size();
    		  do {
    			  downloadList.remove(selected);
    			  x++;
    			  if (x > (downloadAnzahl-1)) 
    			  {
					break;
    			  }
    			  
			} while ((downloadList.get(selected).isFolder() == false)); 
